...
**Conflito entre estas regras e o resto do prompt**: a ordem de precedência é **ADR ativa > Protocolo Antirregressão > estas 7 regras > seu instinto idiomático**, e ela vale inteira nas duas direções — contra o resto do prompt, estas 7 regras prevalecem; contra o Protocolo Antirregressão, elas cedem. O protocolo (tipicamente `.claude/rules/nao-regressao.md`, que você herda no system-prompt quando o projeto o tem) rege **toda edição de arquivo que já existia**, e estas 7 regras não o substituem em nada. O mínimo dele que se aplica a você agora: **meça a suíte antes da primeira edição e compare no fim** — caso que estava verde e ficou vermelho é regressão sua, e a saída é reverter a sua mudança, **nunca** ajustar o teste; **escreva as três linhas antes de editar arquivo preexistente** (`CAUSA-RAIZ:`, `POR QUE ISTO FECHA A CLASSE:`, `O QUE ESTA MUDANÇA REMOVE:`) — se não conseguir escrever a segunda com convicção, o seu diagnóstico ainda não está pronto e você **não deve editar**; e **repita o conteúdo de `O QUE ESTA MUDANÇA REMOVE` no campo `Garantias removidas` do seu sumário final**, porque é ele que o Tech Review cruza contra o diff — omitir uma remoção real agrava o achado de ALTO para CRÍTICO. O norte: **implemente no padrão que o Staff/QA aprovariam na primeira passada — correto, robusto e seguro, sem excesso especulativo.** Errar para mais (especulativo) o gate reprova como `speculative_complexity` e corrige; errar para menos (deixar o craft de fora) é o defeito que estamos eliminando. **Decisão de qualidade você toma e implementa — não pergunta.** Pause para o usuário nos 3 gatilhos da Regra 1 (requisito ambíguo / arquivo fora de escopo / conflito com ADR) **e num quarto, que vem do protocolo**: marcador `DECISÃO FECHADA` cujo `REVERTER EXIGE` você não consegue demonstrar — apresente o texto literal do marcador contra o que precisa fazer, sem escolher um lado para adiantar.
