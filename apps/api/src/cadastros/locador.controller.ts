/**
 * As **seis rotas do locador** — um dos três papéis de cadastro de pessoa, no molde que a T5
 * estabeleceu.
 *
 * ---------------------------------------------------------------------------
 ...
 * ---------------------------------------------------------------------------
 *
 * `'locador'` entra como argumento na construção da superfície, a partir da união fechada
...
/** Caminho da superfície de locadores, relativo ao prefixo de versão (§4.1: `/v1/locadores/…`). */
export const CAMINHO_DOS_LOCADORES = 'locadores';
 ...
 */
const PAPEL = PAPEIS_DE_PESSOA[0];

@ApiTags('locadores')
@Controller(CAMINHO_DOS_LOCADORES)
...
export class LocadorController {
  ...
  @Post()
  @ApiOperation({ summary: 'Cria um locador', description: DESCRICOES.criar })
  @ApiCreatedResponse({
  ...
  @Get()
  @ApiOperation({ summary: 'Lista os locadores da empresa', description: DESCRICOES.listar })
  @ApiOkResponse({
  ...
  @Get(':id')
  @ApiOperation({ summary: 'Lê um locador pelo identificador', description: DESCRICOES.ler })
  @ApiOkResponse({
  ...
  @Put(':id')
  @ApiOperation({ summary: 'Reescreve um locador', description: DESCRICOES.alterar })
  @ApiOkResponse({
  ...
  @ApiOperation({
    summary: 'Retira um locador de circulação',
  ...
  @ExigeChaves(AREA_DO_CADASTRO, ACAO_DE_CIRCULACAO)
  @ApiOperation({
    summary: 'Devolve um locador à circulação',
