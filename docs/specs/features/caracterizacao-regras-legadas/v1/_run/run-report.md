...
### D8 · BAIXO · code_quality · TC-001 · validação da intervenção de 2026-08-08
- **Onde:** `deploy/scripts/caracterizacao/verificar-captura.sh:ler_credencial_db`
- **Problema:** `senha_producao` passa por `tr -d '\r\n'` ao ser lida do `site_config.json`, mas `credencial_db` — que sai do `awk` sobre o `docker-compose.yml` — não recebe tratamento equivalente.
- **Impacto:** nulo hoje e **preexistente** — não foi introduzido nem agravado pela intervenção que fechou o D1, e o comportamento é idêntico ao da varredura de uma agulha só. Num `docker-compose.yml` com terminação CRLF, a agulha carregaria um `\r` final que o `rstrip("\n")` do bloco Python não remove, e o padrão de fronteira deixaria de casar o valor real nos logs — a varredura do `CT-013` passaria **sem conferir de fato** aquela credencial. O arquivo de hoje tem terminação LF, então a condição não se realiza.
- **O que fazer:** `rstrip()` sem argumento no bloco Python, ou um `tr -d '\r\n'` na saída de `ler_credencial_db` no shell — simetria com o tratamento que `senha_producao` já recebe. Registrado por medição do validador independente da intervenção, que o classificou como **não-acionável naquele escopo** (é higiene de leitura de credencial, não o privilégio de autenticação que o D1 tratava).
