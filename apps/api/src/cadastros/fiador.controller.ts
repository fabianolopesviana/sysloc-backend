/**
 * As **seis rotas do fiador** — um dos três papéis de cadastro de pessoa, no molde que a T5
 * estabeleceu.
 *
 * ---------------------------------------------------------------------------
 ...
 * ---------------------------------------------------------------------------
 *
 * `'fiador'` entra como argumento na construção da superfície, a partir da união fechada
...
/** Caminho da superfície de fiadores, relativo ao prefixo de versão (§4.1: `/v1/fiadores/…`). */
export const CAMINHO_DOS_FIADORES = 'fiadores';
 ...
 */
const PAPEL = PAPEIS_DE_PESSOA[2];

@ApiTags('fiadores')
@Controller(CAMINHO_DOS_FIADORES)
@ExigeChave(AREA_DO_CADASTRO)
export class FiadorController {
  ...
  @Post()
  @ApiOperation({ summary: 'Cria um fiador', description: DESCRICOES.criar })
  @ApiCreatedResponse({
  ...
  @Get()
  @ApiOperation({ summary: 'Lista os fiadores da empresa', description: DESCRICOES.listar })
  @ApiOkResponse({
  ...
  @Get(':id')
  @ApiOperation({ summary: 'Lê um fiador pelo identificador', description: DESCRICOES.ler })
  @ApiOkResponse({
  ...
  @Put(':id')
  @ApiOperation({ summary: 'Reescreve um fiador', description: DESCRICOES.alterar })
  @ApiOkResponse({
  ...
  @ApiOperation({
    summary: 'Retira um fiador de circulação',
  ...
  @ExigeChaves(AREA_DO_CADASTRO, ACAO_DE_CIRCULACAO)
  @ApiOperation({
    summary: 'Devolve um fiador à circulação',
