...
# Degradação declarada e diagnóstico rotineiro são coisas diferentes e saem por canais diferentes,
# como no exemplar `verificar-provisionamento.sh`. Nenhuma das duas conta como falha; a diferença
# está em quem precisa vê-las: `aviso` é o que ficou POR FAZER, e vai para a saída de erro — o
# canal que um agregador (T7) reencaminha mesmo quando arquiva a saída padrão em log. `nota` é
# observação de execução, e fica na saída padrão. Colapsar as duas na mesma função faria a
# pendência da CA-14 sair indistinguível de "a instância reportou 18.4".
