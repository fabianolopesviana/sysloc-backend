...
- **Suíte: 1596 casos**, 9 pacotes — `contracts` 398 · `api` 317 · `shared` 236 · `db` 215 ·
  `documentos` 151 · `worker` 99 · `auth` 89 · `cobranca-bancaria` 61 · `regua` 30. ⚠️ **Meça por
  pacote** (`pnpm --filter @sysloc/<p> test`): o `turbo run test` aborta os pacotes irmãos e a saída
  agregada não é confiável.
...
`D26`**, **dois `D27`**, **dois `D28`** e **dois `D49`**, todos legítimos e todos débitos diferentes. A regra completa está na §3-B
...
| **D34** (F4/T11, fatia `emissao-e-conciliacao`) | `packages/cobranca-bancaria/src/emissao-em-lote.ts` (junto de `guarda.gravar`) | a fatia que trouxer a **notícia recebida do provedor** (a (iii)), ou a que ampliar o modelo canônico para perguntar pelo identificador enviado — a T12 mediu que a conferência não alcança o órfão |
