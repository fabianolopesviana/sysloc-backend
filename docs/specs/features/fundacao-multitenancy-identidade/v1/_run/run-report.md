...
- **Examinado e NÃO fechado na intervenção dirigida de 2026-08-22**, com as duas alternativas medidas — o registro fica para que a próxima leitura não refaça a medição:
  - **Mover o export para subcaminho** custa **13 arquivos de teste** em três pacotes (`apps/api` 4, `packages/auth` 8, mais o `folha.spec.ts` que descreve a aresta), o `exports` do manifesto — hoje com um único caminho `"."` —, o barril e a âncora do `CT-012`, que audita a lista de exports **por igualdade**. É refactor cross-module, e a proibição 5 do Protocolo Antirregressão o nomeia.
  - **Guarda de destino por `NODE_ENV`** — o discriminador que o repositório já usa em `packages/auth/src/autenticacao.ts` — fecharia o caminho *"processo de produção"* e deixaria aberto o *script ad-hoc sem `NODE_ENV` contra o banco durável*. Fecha o caso, não a classe.
  - **Por isso não se editou**: o **P3** manda não editar quando a linha `POR QUE ISTO FECHA A CLASSE` não se escreve com convicção, e nenhuma das duas a sustenta. O que já limita o estrago segue valendo e está provado (`semente.spec.ts`, par positivo/negativo): sem a função de derivação, `semear` não escreve credencial alguma.

### D5 · BAIXO · architecture · T2 · Tech Review — ✅ RESOLVIDO (ver `Status:`)
...
### D7 · BAIXO · security/architecture · F1/T6 · Tech Review — **tem marcador** — ✅ RESOLVIDO (ver `Status:`)
...
### D10 · BAIXO · security · T3 · Tech Review — ✅ **RESOLVIDO** (intervenção dirigida de 2026-08-19)
- **Status:** ✅ **RESOLVIDO** — intervenção dirigida de 2026-08-19, pelo caminho que o próprio débito indicou: o **`CT-014` ganhou o segundo eixo**. Nasceram `CT-014 (b)` — a variável `app.empresa_id` é **escrita** (`SET LOCAL` ou `set_config`) em exatamente dois arquivos de produção, por **igualdade de conjunto** contra `unidade-de-trabalho.ts` (a implementação do escritor único) e `semente.ts` (a carga inicial, que usa `set_config` por precisar de parâmetro vinculado) — e `CT-014 (b, falsificação)`, que é a prova exigida por ser asserção **estática**: **controle negativo** (o módulo que só LÊ, `contexto-de-escrita.ts` com `current_setting`, **não** é escritor — um detector que a casasse condenaria toda política de RLS, o defeito literal da `testing-stack.md`) e **controle positivo** nas duas formas de escrita da árvore. É o eixo que o `CT-014` não alcançava e o `CT-326` não cobre: quem recebe o `tx` cru de `emUnidadeDeTrabalho` pode refixar a variável **dentro** da transação já aberta, e a RLS obedece ao valor novo até o `COMMIT`. Um terceiro escritor passa a reprovar nominalmente. Tabela de INVARIANTES do arquivo atualizada. `@sysloc/db` 225 → **227 verdes**.
...
### D21 · BAIXO · security · F1/T7 · Tech Review — **tem marcador** — ✅ RESOLVIDO (ver `Status:`)
...
### D39 · MEDIO · technical_requirement · F1/fechamento · bateria privilegiada — ✅ **FECHADO** na intervenção dirigida de 2026-08-16
...
> ✅ **FECHADO em 2026-08-16, na intervenção dirigida** — e o que o destravou foi a **refutação da premissa**, não uma autorização para seguir sem prova.
>
> **A premissa refutada era** *"a única prova possível é a bateria privilegiada"*. Ela é **falsa desde a T7 da fatia `regua-de-cobranca`**, que instalou no `verificar-provisionamento.sh` o idioma do `CT-647`: as funções do provisionador são carregadas por `eval` do corpo extraído com `sed`, e operam sobre um caminho recebido por parâmetro — *"o `sudo` vem do `main` deste verificador, não deste caso"*. Nada nelas exige privilégio. A premissa sobreviveu duas fases porque foi **herdada por citação** e nunca remedida, que é o mesmo padrão da T1 da fatia `cobranca-e-mora` (*"premissa que bloqueia trabalho merece ser medida antes de ser registrada"*).
>
> **O que foi feito**, nos DOIS caminhos que produzem o arquivo de ambiente:
> - **arquivo criado do zero** — o bloco redirecionado para `${ARQ_AMBIENTE}` passa a emitir as três, com os dois segredos gerados e conferidos antes da escrita;
> - **arquivo preexistente** — nasce `garantir_segredos_do_ambiente`, irmã de `garantir_chaves_de_conteudo` e separada dela de propósito: aquela semeia **valor padrão declarado**, e valor padrão de segredo faria toda instalação nascer com a mesma chave. O critério compartilhado é a **existência da linha**, nunca o valor — linha vazia é deixada como está, e a recusa de partida a nomeia.
>
> **A chave de cifra exigiu gerador próprio, e isso é o achado técnico do fecho.** `gerar_segredo` produz 32 caracteres alfanuméricos, que decodificam para **24 bytes** — o esquema exige 32 e ainda cobra codificação **canônica**. Reusá-la teria produzido uma instalação que só falharia quando a primeira empresa tentasse cobrar. Nasceu `gerar_chave_de_cifra` (32 bytes crus de `/dev/urandom`, base64 inteiro).
>
> **Provas medidas** (todas sem privilégio):
> 1. **Comportamental** — as funções foram carregadas pelo idioma do `CT-647` e o arquivo produzido foi submetido ao validador **real** (`carregarAmbiente`): partida **ACEITA**, com `chaveDeCifraDoCertificado` de exatos **32 bytes**.
> 2. **Idempotência** — segunda execução sobre o mesmo arquivo devolve `SEMEADAS` e `SEGREDOS` vazios, e o `diff` contra a primeira é **vazio**: não regera.
> 3. **Falsificação do gerador** — a chave gerada por `gerar_segredo` e uma com recorte não-canônico são **as duas recusadas**, nomeando a variável; o controle íntegro passa.
> 4. **Falsificação da emissão** — removida a emissão de `BETTER_AUTH_SECRET` do script, o `CT-639` reprova com `expected [ 'BETTER_AUTH_SECRET' ] to deeply equal []`, **nomeando a ofensora**; removidas as outras duas, reprova com as duas. Script restaurado byte a byte nas duas vezes.
>
> **A testemunha `EXIGIDAS_SEM_PROVISIONAMENTO` foi para `[]`** — igualdade exata preservada, jamais `toContain`, e a direção é a **mais forte** (de *"estas três podem faltar"* para *"nenhuma pode"*). O docblock dela passou a proibir explicitamente que ela volte a crescer: exigência nova sem provisionamento é **defeito**, e a correção é ensinar o provisionador — nunca acrescentá-la à lista, que foi o que fez este débito parecer registrado em vez de resolvido.
>
> ⚠️ **O que este fecho NÃO prova, e fica para a sua bateria**: que o script roda de ponta a ponta como root numa máquina nova. As provas acima exercitam as funções e o formato do que elas produzem, não o passo P06 sob privilégio. A confirmação é `sudo bash deploy/scripts/instalacao/verificar-provisionamento.sh` na próxima instalação — que deixou de ser **pré-requisito** do fecho e passou a ser **confirmação** dele.
